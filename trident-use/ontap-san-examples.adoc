...
| `managementLIF` | 集群或SVM管理LIF的IP地址。
...
*对于MetroCluster省略。*请参见 <<mcc-best>>。 | 如果是SVM、则派生 `managementLIF` 已指定 
...
| `snapshotReserve` | 为快照预留的卷百分比 | 如果为"0"、则为"0" `snapshotPolicy` 为"none"、否则为"" 
